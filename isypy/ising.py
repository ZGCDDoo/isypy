...
class Ising(abc_markovchain.ABCMarkovChain):
    ...
    def __init__(self, yy_params) -> None:
        ...
        self.spins = np.random.choice([self.spin_down, self.spin_up], (Nx, Ny, Nz))
        ...
        assert (
            len(self.Jparams) == 3
        ), "Miseria, more than 3 parameters for the J . Stupido !"
        ...
        self.current = {
            "Spin_Flip": {
                "Indices": (self.invalid, self.invalid, self.invalid),
                "DeltaEnergy": self.invalidf,
                "SpinValue": self.invalid,
                "Weight": self.invalid,
            },
    ...
    def try_single_spin_flip(self) -> None:
        ...
        # periodic boundary conditions
        (nxp1, nxm1) = (
            nx + 1 if nx != (Nx - 1) else 0,
            nx - 1 if nx != 0 else (Nx - 1),
        )

        (nyp1, nym1) = (
            ny + 1 if ny != (Ny - 1) else 0,
            ny - 1 if ny != 0 else (Ny - 1),
        )

        (nzp1, nzm1) = (
            nz + 1 if nz != (Nz - 1) else 0,
    ...
    def accept_flip(self) -> None:
        ...
        self.spins[self.current["Spin_Flip"]["Indices"]] = self.current["Spin_Flip"][
            "SpinValue"
        ]
